...
def main():
    ...
    if (
        problem == 'P9_single'
        or problem == 'P9_double'
        or problem == 'P9_triple'
    ):
...
def plot_colormaps(U, problem):
    ...
    ax1.contourf(x, y, np.real(U_0).T)
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    cb1 = fig.colorbar(ax1.contourf(x, y, np.real(U_0).T), ax=ax1)
    cb1.formatter.set_powerlimits((0, 0))

    cb1.set_label('Re(U(t=0))')

    ax1.set_title('Real part')
    ax2.contourf(x, y, np.imag(U_0).T)
    ax2.set_title('Imaginary part')
    ax2.set_xlabel('x')

    cb2 = fig.colorbar(ax2.contourf(x, y, np.imag(U_0).T), ax=ax2)
    ...
    ax1.contourf(x, y, np.real(U_1).T)
    ax1.set_title('Real part')
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    cb1 = fig.colorbar(ax1.contourf(x, y, np.real(U_1).T), ax=ax1)
    cb1.formatter.set_powerlimits((0, 0))
    ...
    cb2 = fig.colorbar(ax2.contourf(x, y, np.imag(U_1).T), ax=ax2)
    cb2.formatter.set_powerlimits((0, 0))
    ...
    ax1.contourf(x, y, np.real(U_2).T)
    ax1.set_title('Real part')
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    cb1 = fig.colorbar(ax1.contourf(x, y, np.real(U_2).T), ax=ax1)
    cb1.formatter.set_powerlimits((0, 0))
    ...
    cb2 = fig.colorbar(ax2.contourf(x, y, np.imag(U_2).T), ax=ax2)
    cb2.formatter.set_powerlimits((0, 0))
    ...
    ax1.contourf(x, y, P_0.T, levels=np.linspace(0, m, 20))
    ax1.set_title('t=0', fontsize=fontsize)
    ax1.set_xlabel('x', fontsize=fontsize)
    ax1.set_ylabel('y', fontsize=fontsize)
    ax1.tick_params(axis='both', labelsize=fontsize)

    cb = fig.colorbar(ax1.contourf(x, y, P_0.T), ax=ax1, label='P')
    ...
    m = np.max(P_1)
    ax2.contourf(x, y, P_1.T, levels=np.linspace(0, m, 20))
    ax2.set_title('t=0.001', fontsize=fontsize)
    ax2.set_xlabel('x', fontsize=fontsize)
    ax2.tick_params(axis='both', labelsize=fontsize)

    cb1 = fig.colorbar(ax2.contourf(x, y, P_1.T), ax=ax2, label='P')
    cb1.formatter.set_powerlimits((0, 0))
...
def plot_detector(P_cube, problem):
    ...
    plt.plot(
        y_points,
        P_cube[idx_detector, :, -1] / np.sum(P_cube[idx_detector, :, -1]),
    )
