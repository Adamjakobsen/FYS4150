...
arma::mat jacobi(arma::mat A, int k, int l, int N, std::string ask, double epsilon = std::pow(10, -8))
{
    ...
    std::ofstream outfile("data_P5.txt"); //*** open the outfile to write the data file, something is wonky bonky idk :(
    ...
    if (ask == "val")
    {
        ...
        outfile << "N: " << int N << ",  " << "iterations: " << iteration << "\n"<< std::endl;  //*** commit the iterations and shit into the data file
...
void calc(int N) // *** I made what used to be main a function that calcs our shit and prints
{
    ...
    //*** our prints are commented out for debugging purposes, I intended to uncomment it later
...
int main()
{
   ...
   //could add a loop here *** where N is a list of selected values we try out ***
    calc(6);
    calc(8);
    calc(10);
    calc(12);

    outfile.close("data_P5.txt");  //its the outfile stuff that is fucking up ***
